...
select pa.p_date 판매일자, pa.p_code 상품코드, pr.p_name 상품명, pr.p_price 상품가,
        pa.p_qty 구매수량, pa.p_total 총금액, 
        pa.p_total*100 적립포인트, 
        CASE
            when SUBSTR(pa.p_date, 5, 2) = '01' and SUBSTR(pa.p_date, 7, 2) = 01 then  pa.p_total*200
            else pa.p_total*100
        END 새해2배적립포인트, 
        g.gname 사은품명,
        TO_CHAR(g.g_start,'9,999,999') 포인트START,
        TO_CHAR(g.g_end,'9,999,999') 포인트END
