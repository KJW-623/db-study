...
--2. Student 테이블의 tel 컬럼을 사용하여 1 전공번호(deptno1)가 201번인 학생의 이름과 전화번호, ‘)‘ 가 나오는 위치를 출력하세요.
select * from student;
select name, tel,
       INSTR(tel, ')') 괄호위치
from student
...
--3. Student 테이블에서 1 전공이 101번인 학생들의 tel 컬럼을 조회하여 3이 첫 번째로 나오는 위치를 이름과 전화번호와 함께 출력하세요.
select * from student;
select name, tel,
       INSTR(tel, '3') 괄호위치
from student
