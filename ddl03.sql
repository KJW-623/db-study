...
CREATE TABLE tt04
(
    no NUMBER(3) Primary Key, --PK 한 개만 존재 제약 때문에 설정 불가
    id VARCHAR2(32) Primary Key, --PK 한 개만 존재 제약 때문에 설정 불가
    name VARCHAR2(16) NOT NULL,
    birth DATE 
);
...
CREATE TABLE tt04
(
    no NUMBER(3), --PK 한 개만 존재 제약 때문에 설정 불가
    id VARCHAR2(32), --PK 한 개만 존재 제약 때문에 설정 불가
    name VARCHAR2(16) NOT NULL,
    birth DATE 
...
외래키 FK : 다른테이블에 있는 키값을 참조하는 컬럼
           ...
           외래키 설정된 부모테이블에서 자신을 참조하는 자식테이블의 데이터가 존재하면,
             삭제가 불가능
...
INSERT INTO T_MEMBER VALUES (5, '이름5', 8); 
    --FK에 해당하는 T_CLUB에 id에 8이 존재하지 않아서 불가능
...
select *
--delete
from t_member   -- 외래키 설정된 성태로 참조하는 데이터를 먼저 삭제
where id=5;
